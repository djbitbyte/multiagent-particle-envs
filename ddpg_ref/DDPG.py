...
class DDPG:
    def __init__(self,
        ...
        if load_models is None:
            ...
            self.critics = [Critic(dim_obs, dim_act) for (dim_obs, dim_act) in zip(dim_obs_list, dim_act_list)]
    ...
    def update_policy(self):
        ...
        for agent in range(self.n_agents):
            ...
            current_Q = self.critics[agent](whole_state[:, index_obs:(index_obs+self.dim_obs_list[agent])],
                                            whole_action[:, index_act:(index_act+self.dim_act_list[agent])])
            ...
            target_Q = self.critics_target[agent](
                next_states_batch.view(-1, self.dim_obs_sum)[:, index_obs:(index_obs+self.dim_obs_list[agent])],
                next_actions.view(-1, self.dim_act_sum)[:, index_act:(index_act+self.dim_act_list[agent])]
            )
            ...
            actor_loss = -self.critics[agent](state_i, action_i)
