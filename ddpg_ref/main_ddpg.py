...
parser.add_argument("--snapshot_path", type=str, default="/home/janet/dev/snapshot/ddpg_ref/",
                    help="Path to output model snapshots")
parser.add_argument("--snapshot_prefix", type=str, default="ddpg_reference_latest_episode_",
...
parser.add_argument("--batch_size", type=int, default=1024,
                    help="batch size")
parser.add_argument("--n_episode", type=int, default=500000,
...
FloatTensor = th.cuda.FloatTensor if ddpg.use_cuda else th.FloatTensor

writer = SummaryWriter()

# learning rate decay
if lr_decay:
    scheduler_critic = [StepLR(x, step_size=10000, gamma=0.1) for x in ddpg.critic_optimizer]
    scheduler_actor = [StepLR(x, step_size=10000, gamma=0.1) for x in ddpg.actor_optimizer]
...
for i_episode in range(n_episode):
    ...
    for t in range(max_steps):
        ...
        action = ddpg.select_action(obs_var)      # action in Variable
        ...
        ddpg.memory.push(obs, action, obs_, reward)  # store in Tensor

        obs = obs_

        critics_grad, actors_grad = ddpg.update_policy()

        if ddpg.episode_done > ddpg.episodes_before_train:
    ...
    if (i_episode % consistency_interval) == consistency_interval - 1:
        for agent_i in range(n_agents):
            ...
            writer.add_scalar('communication/agent{}_det'.format(agent_i),
                              ...
                              i_episode)
            det = np.linalg.det(normalized_agent_mapping)
            for goal_i in range(3):
    ...
    if i_episode != 0 and i_episode % snapshot_interval == 0:
        print('Save models!')
        if ddpg.action_noise == "OU_noise":
            states = {'critics': ddpg.critics,
                      'actors': ddpg.actors,
                      'critic_optimizer': ddpg.critic_optimizer,
                      'actor_optimizer': ddpg.actor_optimizer,
                      'critics_target': ddpg.critics_target,
                      'actors_target': ddpg.actors_target,
                      'var': ddpg.var,
                      'ou_prevs': [ou_noise.x_prev for ou_noise in ddpg.ou_noises]}
        else:
            states = {'critics': ddpg.critics,
                      'actors': ddpg.actors,
                      'critic_optimizer': ddpg.critic_optimizer,
